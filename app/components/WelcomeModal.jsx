...
export default function WelcomeModal() {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl flex flex-col sm:flex-row w-full max-w-4xl relative overflow-hidden" style={{maxHeight: '95vh'}}>
        {/* Close Button */}
        <button
          onClick={() => setIsOpen(false)}
          className="absolute top-2 right-2 sm:top-4 sm:right-4 z-20 text-gray-400 hover:text-gray-700 bg-white/80 hover:bg-white rounded-full p-1.5 sm:p-2 text-xl sm:text-2xl leading-none transition-all duration-300"
          aria-label="Close"
        >
          ×
        </button>
        ...
        <div className="w-full sm:w-1/2 bg-gray-100 flex flex-col justify-between relative">
          <div className="relative w-full h-48 sm:h-64 md:h-full" style={{minHeight: '300px', maxHeight: '50vh'}}>
          ...
          <div className="relative bg-white/95 p-4 sm:p-6 text-center">
            <div className="text-xl sm:text-2xl font-bold mb-1 sm:mb-2 text-gray-800">{activeSlides[currentSlide].title}</div>
            <div className="text-sm sm:text-base font-medium mb-1 sm:mb-2 text-gray-700">{activeSlides[currentSlide].description}</div>
            {activeSlides[currentSlide].tnc && (
              <div className="text-xs sm:text-sm text-gray-500">{activeSlides[currentSlide].tnc}</div>
            )}
          </div>

          {/* Dots */}
          <div className="relative bg-white/95 pb-4 sm:pb-6 pt-2 sm:pt-3 flex justify-center gap-2 sm:gap-3">
        ...
        {/* Right: Form */}
        <div className="w-full sm:w-1/2 bg-white p-4 sm:p-8 flex flex-col justify-center">
          {/* Tabs */}
          <div className="flex mb-4 sm:mb-8 bg-gray-100 rounded-full overflow-hidden">
            <button
              className={`flex-1 py-3 sm:py-4 px-3 sm:px-4 text-center text-sm sm:text-base font-semibold transition-all duration-300 focus:outline-none ${
                activeTab === 'personal' 
          ...
          {/* Input */}
          <label className="block text-gray-700 text-sm sm:text-base font-medium mb-2 sm:mb-3" htmlFor="mobile">
            Mobile Number
          </label>
          <div className="flex items-center border-2 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 mb-4 sm:mb-6 bg-gray-50 focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-blue-500">
            <span className="flex items-center mr-2 sm:mr-3 pointer-events-none">
              <span className="text-xl sm:text-2xl">🇮🇳</span>
              <span className="ml-1 sm:ml-2 text-gray-700 font-semibold text-sm sm:text-base">+91</span>
            </span>
            <input
              id="mobile"
              type="tel"
          ...
          {/* Continue Button */}
          <button
            className={`w-full py-3 sm:py-4 rounded-xl text-base sm:text-lg font-semibold mb-4 sm:mb-6 transition-all duration-300 ${
          ...
          <div className="flex items-center my-4 sm:my-6">
            ...
            <span className="mx-3 sm:mx-4 text-gray-500 text-xs sm:text-sm">Or Login/Signup With</span>
            <div className="flex-grow border-t border-gray-300"></div>
          </div>

          <div className="flex justify-center gap-4 sm:gap-6 mb-4 sm:mb-6">
            <button className="p-2.5 sm:p-3 border-2 border-gray-300 rounded-full hover:bg-gray-100 transition-all duration-300" aria-label="Login with Google">
            ...
            <button className="p-2.5 sm:p-3 border-2 border-gray-300 rounded-full hover:bg-gray-100 transition-all duration-300" aria-label="Login with Email">
